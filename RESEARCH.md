...
## 2. Current Phantom Capabilities

### Existing Hound-Adjacent Features
...
| `archer` agent | Cross-file regression: removed handlers, dropped imports, dead code | Post-hoc only, no proactive detection |
| `ward` agent | Witness regression markers in `witness-fixes.json` | Tracks known fixes, not unknown bugs |
| `apex` | Failure classification (build/type/test/ui/visual/integration) | Classifies but doesn't investigate |
...
### Integration Points for Hound Mode
1. **Phase A of start.md** — if input is a bug report, branch into hound flow
...
5. **Standalone command** — `phantom:hound` or `phantom:investigate`

## 3. Design: Hound Mode for Phantom

### Philosophy
Forensic-skills treats analysis as a standalone activity. We treat it as **integrated intelligence** — hound mode activates automatically when symptoms appear, produces structured artifacts, and feeds into the existing fix/verify loop.
...
│  phantom:hound — full forensic analysis         │
│  Structured output → session artifacts           │
├─────────────────────────────────────────────────┤
│  Layer 3: Forensic Knowledge (shared context)    │
│  _shared-hound.md — formulas, thresholds     │
│  Loaded only when hound mode activates       │
└─────────────────────────────────────────────────┘
```
...
If detected → inject hound pre-scan before planning:
```
Phase A.5: Hound Pre-Scan
  ...
  4. Add findings to context.json as `hound` field
...
Pre-Fix Hound Scan
  ...
  4. Add hound findings to verification.json
...
### Layer 2: Investigation Engine (phantom:hound)

New T2 command: `commands/hound.md`

**Trigger conditions:**
- "investigate", "why is X broken", "debug", "trace", "root cause"
- Explicit `/phantom:hound`
...
### Layer 3: Shared Context (_shared-hound.md)

Loaded at T2+ only when hound mode activates. Contains:
...
### Layer Integration with Existing Phantom

```
phantom:start ─── bug detected? ──→ hound pre-scan ──→ plan with evidence
                    │
phantom:verify ── failure? ──→ hound scan ──→ enriched fix context
                    │
phantom:fix ──── loop 2+? ──→ deep investigation ──→ new hypothesis
                    │
phantom:hound ──── standalone investigation ──→ investigation.json
```

### Differences from forensic-skills

| Aspect | forensic-skills | Our Hound Mode |
|--------|----------------|-------------------|
...
**Phase 1: Core Hound Command** (hound.md + _shared-hound.md)
...
- Trigger: explicit `/phantom:hound` or "investigate"
...
- Each injects hound findings into existing artifacts
...
- `commands/verify.md` — add pre-fix hound scan on failure
