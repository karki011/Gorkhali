...
Apply coding principles from repo's `.claude/rules/coding-principles.md` or `{PLUGIN_ROOT}/reference/coding-principles.md` (self-resolve {PLUGIN_ROOT}: `PR="$(ls -dt "$HOME"/.claude/plugins/cache/gorkhali/gorkhali/*/ 2>/dev/null | head -1)"; PR="${PR%/}"; [ -n "$PR" ] && cat "$PR/reference/coding-principles.md"` — empty `$PR` skips the read silently). Priority: KISS > codebase-first > smallest blast radius.
