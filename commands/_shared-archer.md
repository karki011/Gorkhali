...
**Protocol:** Gather graph context (git diff + code-review-graph) → spawn Archer agent (`subagent_type: "archer"`, `name: "archer-sylas"` per `reference/roster.md`; opus — pinned review tier, the top tier now that Fable is retired from Phantom's routing) → merge findings with power level → auto-triage P0/P1 only.
