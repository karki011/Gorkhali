...
LEARNINGS       = {TEAM_DIR}/learnings/           # Domain files: ui.md, data.md, auth.md, testing.md, shadows.md, migration.md, tooling.md
...
| 2 | **Verification mandatory** — Ward runs commands, reads output, confirms pass | "Tests pass" without evidence is the #1 agent failure mode. Evidence-before-assertions. |
...
| 4 | **Rival** — every plan challenged. PROCEED/REVISE/RETHINK | Catches wrong assumptions before expensive implementation. Cheap insurance. |
...
- [ ] Rival reviewed the plan?
- [ ] Simplify ran on all changed files?
- [ ] Apex checked intent alignment (not just test pass)?
- [ ] Learnings recorded?
- [ ] ALL implementation went through the Agent tool — Apex never edited directly?
...
| Tier | Description | Shared Contexts Loaded | Core Rules Active |
|------|-------------|----------------------|-----------------|
| **T1** | Leaf commands — read-only or single action | `_shared.md` only (Governance + Core Rules + Context Mgmt) | 1-3 only |
| **T2** | Verification — run checks, diagnose, report | + `_shared-repo-detection.md` + `_shared-auto-learning.md` | 1-3, 5, 11 |
| **T3** | Planning — research, plan, discuss, review | + `_shared-shadows.md` + `_shared-discipline.md` + `_shared-contracts.md` | All except 12-13 |
...
| **T2** | `verify`, `fix`, `validate`, `eval`, `hound` |
...
|------|----|----|----|----|---------|
| `_shared.md` | Y | Y | Y | Y | Core governance, Core Rules, paths, context management |
...
| `_shared-shadows.md` | | | Y | Y | Agent spawning, shadows roles |
| `_shared-discipline.md` | | | Y | Y | Plan/execution discipline |
| `_shared-contracts.md` | | | Y | Y | Contract templates, hooks |
| `_shared-hound.md` | | Y* | | Y | Forensic analysis (loaded on hound trigger) |
