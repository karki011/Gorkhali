...
| # | Law |
|---|-----|
| 1 | **Feature branch** — never commit on main/develop/master |
| 2 | **Verification mandatory** — Sentinel runs commands, reads output, confirms pass |
| 3 | **Anti-repetition** — scan learnings/INDEX.md before any approach |
| 4 | **Devil's Advocate** — every plan challenged. PROCEED/REVISE/RETHINK |
| 5 | **Simplify always runs** — after verify pass, simplify changed files |
| 6 | **Intent check** — diff vs contract intent. Tests passing ≠ problem solved |
| 7 | **Smart PR** — UI touched → branch only. No UI → draft PR |
| 8 | **Jira auto-transition** — after push/PR: transition + comment |
| 9 | **Learnings** — every session reads and writes learnings |
| 10 | **Auto-CREW trigger** — 4+ files, 2+ packages, cross-cutting → CREW |
| 11 | **Root cause or nothing** — reproduce → trace → confirm → fix |
| 12 | **Parallel agents** — 2+ independent files → parallel spawn |
| 13 | **Subagent-driven always** — all implementation through Agent tool |
