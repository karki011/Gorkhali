...
5. **Enriched-artifact check (mandatory, plan-gate only)** — when `{ARTIFACT}` is a rendered `plan.html`, verify it leads with the research narrative (question, findings with file:line evidence, verdict, decisions) and isn't a bare file/task manifest. If it fails — findings buried, absent, or tasks/files rendered first — rewrite/enrich the artifact per the Research-Enriched Plan Artifact rule in `reference/planning.md` FIRST, then proceed to Step 2. Never open a manifest-only plan gate on a research ticket.
