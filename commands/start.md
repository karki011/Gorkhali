...
1. Intent (Chief writes `intent.json`) → **spawn `planner-drafton`, draft pass** (`research` profile, blocking, spawn spec in `reference/planning.md` → Codebase Research). The planner researches the codebase and drafts the decision-first `plan.json` (`_meta.version: 3`, no HTML yet, `oppositionVerdict` unset) per `reference/agents.md` and `reference/schemas/plan.md`: briefing (`tackling`, `problem`, `how`), decision, outcome, scope, architecture, evidence, alternatives, risks, validation, and task contracts must be complete before the gate. For standard/deep plans, require decision implications, substantive tradeoffs, risk triggers/recovery, and executable task dossiers; populated-but-generic fields do not pass the gate. A How without supporting evidence is an assumption. A refuted or stale premise still stops planning. Chief ingests `jq '.briefing, .decision, .outcome, .scope, .evidence, .risks, .open_questions, .assumptions, .alternatives, .oppositionVerdict, [.tasks[] | {id, files, dependsOn}]'` - every field the gate presents, plus the task table shown only on request - and never Reads `plan.json` whole.
2. Deliberation: planner ↔ Opposition (`opposition-parlow`, the one plan critic, writes `plan-check.json`), 2 rounds per `reference/router/deliberation.md`. Opposition returns its verdict line only. After EVERY verdict, PROCEED included, Chief re-spawns `planner-drafton` for the **finalize pass** with the `plan-check.json` PATH, not its contents: it records `oppositionVerdict` and addresses challenges on REVISE. The finalize pass returns the verdict plus the same brief fields Chief already ingests via the jq projection in step 1, so the gate in step 3 has evidence, scope, risks, and open questions to present. A REVISE round therefore runs finalize → Opposition → finalize.
3. **HUMAN GATE**: approve plan (`--to-plan` mode: this gate is replaced per `## Mode: --to-plan`). The chat brief is the only presentation of this gate: no page, no artifact. Write it in plain English for an engineer from another team - one or two sentences per field, leading with the conclusion, keeping file and symbol names out of the prose and attached instead as a trailing code chip, and never simplified into something false. Collect approval and feedback in chat using this brief - never tasks:
   ...
   Material feedback goes back to `planner-drafton` (re-spawn with the feedback and the `plan.json` path), which updates `plan.json`, and Opposition reruns before fresh generation; presentation-only feedback leaves JSON unchanged and the planner regenerates from the same source plus that feedback. Chief never edits either file. Never degrade to a task-only gate.
...
**EXIT:** `planner-drafton` (draft pass, then the finalize pass after the inline Opposition review) writes `plan.json` to the session dir with `oppositionVerdict` recorded, for the human who reviews it later as the same chat brief. Then print exactly one report line — `[PLANNED] {TICKET} — {N} files, {summary}` — and STOP. Prohibited in this mode: Engineer implementation spawns, verify, fix, wrap, git mutations, worktree creation, or opening browsers.
