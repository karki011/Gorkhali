...
- **Phase A + B:** Chief gathers context and classifies. This is coordinator work — call MCP tools, grep learnings, write `intent.json` / `route-decision.json` / `context.json`. It is NOT reading project source: that is the planner's job (`reference/planning.md` → Codebase Research, `research` profile). Phase B classification signals come from `git`, `wc`, and `grep -l` counts, not from opening files.
- **Implementation:** ALWAYS spawn Engineer agent(s) via the Agent tool. Chief writes `intent.json` and `contracts/`; the planner writes `plan.json` and the review HTML; nobody but an Engineer edits project source files.
...
1. Intent (Chief writes `intent.json`) → **spawn `planner-drafton`, draft pass** (`research` profile, blocking, spawn spec in `reference/planning.md` → Codebase Research). The planner researches the codebase and drafts the decision-first `plan.json` (`_meta.version: 3`, no HTML yet, `oppositionVerdict` unset) per `reference/agents.md` and `reference/schemas/plan.md`: briefing (`tackling`, `problem`, `how`), decision, outcome, scope, architecture, evidence, alternatives, risks, validation, and task contracts must be complete before the gate. For standard/deep plans, require decision implications, substantive tradeoffs, risk triggers/recovery, and executable task dossiers; populated-but-generic fields do not pass the gate. A How without supporting evidence is an assumption. A refuted or stale premise still stops planning. Chief ingests `jq '.briefing, [.tasks[] | {id, files, dependsOn}]'` and never Reads `plan.json` whole.
2. Deliberation: planner ↔ Opposition (`opposition-parlow`, the one plan critic, writes `plan-check.json`), 2 rounds per `reference/router/deliberation.md`. Opposition returns its verdict line only. After EVERY verdict, PROCEED included, Chief re-spawns `planner-drafton` for the **finalize pass** with the `plan-check.json` PATH, not its contents: it records `oppositionVerdict`, addresses challenges on REVISE, then authors and promotes the review page (it does not publish it). A REVISE round therefore runs finalize → Opposition → finalize.
3. **HUMAN GATE**: approve plan (`--to-plan` mode: this gate is replaced per `## Mode: --to-plan`). The finalize pass has already validated `plan.json` and authored `{SESSION_DIR}/plan.candidate.html` from the canonical JSON and the sibling `plan-check.json`, so the page carries Opposition provenance. The planner chooses the information design within `skills/gorkhali/references/review-html.md`: plain-English voice, decision on top, mechanics collapsed at the bottom. The page leads with What (`briefing.tackling`), Problem (`briefing.problem`), and How (`briefing.how`), then evidence, scope, risks, and open questions. Implementation (files, tasks, waves) lives in a collapsed `<details>` appendix with no `open` attribute. Only a valid candidate is promoted, with `node {PLUGIN_ROOT}/skills/gorkhali/scripts/validate-review-html.mjs plan --source {SESSION_DIR}/plan.json --candidate {SESSION_DIR}/plan.candidate.html --out {SESSION_DIR}/plan.html --target artifact`; Chief checks the validator exit code the planner reported, and reruns the command itself only if that report is missing. `plan.json` stays the machine SSoT; the page is disposable, never parsed back, and never manually patched or repaired. **Chief then publishes it itself**: `Artifact(file_path: "{SESSION_DIR}/plan.html", favicon: "<one emoji>", description: "<one sentence: what this plan decides>")`, then gives the user the returned URL. Republishing the same `file_path` on later rounds keeps one stable URL for the session; omit `favicon` on a republish. If the publish fails or is declined, regenerate with `--target file` and open the local page instead, and say which happened. Then collect approval and feedback in chat using this brief - never tasks:
   ...
   - **Open questions**
   - **Approve?**
   Material feedback goes back to `planner-drafton` (re-spawn with the feedback and the `plan.json` path), which updates `plan.json`, and Opposition reruns before fresh generation; presentation-only feedback leaves JSON unchanged and the planner regenerates from the same source plus that feedback. Chief never edits either file. Validate/promote the fresh candidate and republish to the same `file_path` only when the user asks to review it again. If candidate generation, validation, publishing, or opening is unavailable, present the same What/Problem/How brief in chat and state the capability failure. Never present a URL a publish result did not return, and never degrade to a task-only gate.
...
**EXIT:** `planner-drafton` (draft pass, then the finalize pass after the inline Opposition review) writes `plan.json` to the session dir with `oppositionVerdict` recorded, then best-effort authors `{SESSION_DIR}/plan.candidate.html` and run `node {PLUGIN_ROOT}/skills/gorkhali/scripts/validate-review-html.mjs plan --source {SESSION_DIR}/plan.json --candidate {SESSION_DIR}/plan.candidate.html --out {SESSION_DIR}/plan.html --target artifact` for the human who reviews later. Chief publishes the accepted page with `Artifact(file_path: "{SESSION_DIR}/plan.html", ...)` and includes the URL in the report line. Candidate generation, validation, or publishing failure is non-blocking and does not stop the exit. Then print exactly one report line — `[PLANNED] {TICKET} — {N} files, {summary}` — and STOP. Prohibited in this mode: Engineer implementation spawns, verify, fix, wrap, git mutations, worktree creation, or opening browsers.
