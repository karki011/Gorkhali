...
5. **Spawn Lens** (model: sonnet, mode: bypassPermissions): target routes + backend + `lens-qa` session + expected behavior from contract/intent.
...
2. Apex dispatches Blade (UI focus) scoped to affected files — appearance only, not behavior
3. Re-run correctness on fixed files. If fails → revert, mark "needs manual fix"
