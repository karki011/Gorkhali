...
const { scoreRisk, tierForTask, buildExecutionWaves, THRESHOLDS, isolationForTask } = require('./routing');
const { modelForTier } = require('./tiers');
const { integrate, releaseWorktrees, readJournal, completedRecords, taskHash, ownershipReport, dirtyOutsideOwnership } = require('./execution');
  ...
  switch (action) {
    ...
    case 'status': {
      ...
      const preview = classification === 'divergent' ? { ...saved, verification: 'stale', reconciliationRequired: true, next: 'reconcile' }
        ...
        : saved;
      return { checkpoint: preview, current, tracking: tracking.read(dir), progress: session.readProgress(repo, task, cwd), worktrees: worktreeReport(false) };
    }
    ...
    case 'route': {
      ...
      const activeEngineers = saved?.activeEngineers || [];
      return { waves, auditor: { tier: reviewerTier, model: modelForTier(reviewerTier), risk: scoreRisk(combined) },
        ...
        const wave = waves.find((ids) => ids.includes(item.id)) || [];
        const pending = waves.slice(waves.indexOf(wave)).flat();
        const isolation = isolationForTask(item, wave, value, { pending, activeEngineers, ...counters });
        return { id: item.id, risk: scoreRisk(risk), tier, model: modelForTier(tier), isolation };
      }) };
    }
    case 'dispatch': {
      approved(); reconciled();
      ...
      const pending = remaining.map((item) => item.id);
      const activeEngineers = saved?.activeEngineers || [];
      const counters = { implementationFailures: saved?.implementationFailures || 0, verificationFailures: saved?.verificationFailures || 0 };
      const routing = wave.map((id) => {
        ...
        const tier = tierForTask('engineer', risk);
        const isolation = isolationForTask(item, wave, value, { pending, activeEngineers, ...counters });
        return { id, risk: scoreRisk(risk), tier, model: modelForTier(tier), isolation };
      });
      // Wave computation already forces worktree isolation whenever more than one task
      // is scheduled together; this is a defensive rail against a future regression.
      if (wave.length !== 1 && routing.some((item) => item.isolation === 'branch')) throw new Error('Branch-mode dispatch requires a single-task wave');
    ...
    case 'result': {
      ...
      // A non-done branch-mode result may leave un-journaled commits (or a dirty tree) sitting
      // directly on the integration branch; force reconciliation before recover/integrate touch it.
      if (assignment.isolation === 'branch' && record.status !== 'done') {
        const current = snapshot(cwd);
        ...
        if (!descendsFromBase) {
          ...
          entry.reconciliationReason = `Integration branch no longer descends from the branch-mode base ${assignment.baseHead}`;
          ...
          if (clauses.length) {
            entry.reconciliationRequired = true;
            entry.reconciliationReason = `Branch-mode attempt ${record.attemptId} ${clauses.join(' ')}`;
            entry.next = 'reconcile';
          }
        }
      }
    ...
    case 'recover': {
      ...
      if (result.next === 'engineer') {
        ...
        extra.assignments = [{ id: repairTask.id, attemptId: randomUUID(), baseHead: state.head, taskHash: taskHash(repairTask), tier, model: result.model, isolation: 'worktree' }];
    ...
    case 'integrate': {
      ...
      for (const record of input.records || []) {
        ...
        const expectedIsolation = assignment.isolation || 'worktree';
        const actualIsolation = record.isolation || 'worktree';
        if (expectedIsolation !== actualIsolation) throw new Error(`Isolation mode mismatch: expected ${expectedIsolation} for task ${record.taskId}`);
      }
