...
const { integrate, releaseWorktrees, readJournal, completedRecords, taskHash, ownershipReport } = require('./execution');
    ...
    case 'result': {
      ...
      if (assignment.isolation === 'branch' && record.status !== 'done') {
        ...
        if (!descendsFromBase) {
          ...
          if (commits.length || current.dirtyFiles.length) {
            const ownership = outside.length ? `outside declared ownership: ${outside.join(', ')}` : 'all within declared ownership';
            entry.reconciliationRequired = true;
            entry.reconciliationReason = `Branch-mode attempt ${record.attemptId} left commit(s) [${commits.join(', ')}] and dirty files [${current.dirtyFiles.join(', ')}] on the integration branch; ${ownership}`;
            entry.next = 'reconcile';
          }
        }
      }
