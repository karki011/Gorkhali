...
function run(action, input = {}, cwd = process.cwd()) {
  ...
  const worktreeReport = (apply) => {
    try { return releaseWorktrees(dir, cwd, apply); } catch (err) { return { released: [], kept: [], unintegrated: [], error: String(err.stderr || err.message).trim() }; }
  };
  ...
  switch (action) {
    ...
    case 'ship': {
      ...
      return { url, worktrees: worktreeReport(true) };
    }
    case 'review-state': {
