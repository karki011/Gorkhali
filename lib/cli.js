...
const { integrate, readJournal, completedRecords, taskHash } = require('./execution');
...
function run(action, input = {}, cwd = process.cwd()) {
  ...
  switch (action) {
    ...
    case 'status': {
      ...
      const saved = session.readCheckpoint(repo, task, cwd);
      return { checkpoint: saved && saved.fingerprint !== current.fingerprint ? { ...saved, verification: 'stale', reconciliationRequired: true, next: 'reconcile' } : saved, current, tracking: tracking.read(dir), progress: session.readProgress(repo, task, cwd) };
    }
