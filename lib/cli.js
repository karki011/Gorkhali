...
function run(action, input = {}, cwd = process.cwd()) {
  ...
  switch (action) {
    ...
    case 'plan': {
      ...
      if (previous?.verification === 'passed') return { ...input.plan, notice: 'Plan changed after verification: Inspector, Auditor, and human evidence no longer apply. Batch further amendments before verifying again at wrap.' };
      return input.plan;
    }
    case 'approve': {
      const value = plan();
