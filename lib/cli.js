...
const { git, snapshot, digest, originDefaultBranch } = require('./git-state');
...
const { integrate, completedRecords, taskHash, ownershipReport, dirtyOutsideOwnership } = require('./execution');
...
function run(action, input = {}, cwd = process.cwd()) {
  ...
  switch (action) {
    ...
    case 'plan': {
      ...
      if (errors.length) throw new Error(errors.join('; '));
      const previous = session.readCheckpoint(repo, task, cwd);
      if (previous?.activeEngineers?.length) throw new Error('Stop or collect active Engineers before changing the plan');
      session.writePlan(repo, task, input.plan, cwd);
      // Surface the cost at the decision point: every amendment after verification forces a full re-verification.
      if (previous?.verification === 'passed') return { ...input.plan, notice: 'Plan changed after verification: mechanical and review evidence need refreshing. Visual confirmation can carry forward when its acceptance scope is unchanged. Batch further amendments before verifying again at wrap.' };
      return input.plan;
    }
    case 'approve': {
      const value = plan();
      const saved = session.readCheckpoint(repo, task, cwd);
      const planHash = digest(JSON.stringify(value));
      const state = snapshot(cwd);
      let approval = { mode: 'human', planHash };
      if (input.autonomous === true) {
        const reason = autonomy.eligibility(value);
        if (reason) throw new Error(reason);
        if (saved?.approval?.mode === 'autonomous' && saved.approvedPlanHash === planHash) return saved;
        if (saved?.approvedPlanHash || saved?.approval || saved?.reconciliationRequired || saved?.activeEngineers?.length || saved?.pendingFailureIds?.length) throw new Error('Existing or changed scope requires explicit human approval');
        if (state.dirtyFiles.length || ['main', 'master', 'HEAD'].includes(state.branch)) throw new Error('Autonomous work requires a clean feature branch');
        if (state.branch === originDefaultBranch(cwd)) throw new Error('Cannot approve autonomous work on the origin default branch');
        if (value.baseHead !== state.head) throw new Error('Autonomous plan base must match the current HEAD');
        const tracked = tracking.read(dir);
        if (value.autonomy.ticketProvided && tracked.decision !== 'ticket') throw new Error('Bind and read the supplied ticket before autonomous work');
        if (!value.autonomy.ticketProvided && tracked.decision === 'unanswered') {
          session.writeJsonAtomic(path.join(dir, 'tracking.json'), { decision: 'none', source: 'autonomous-policy', reason: 'Well-scoped task supplied without a ticket', stages: {} });
        }
        approval = { mode: 'autonomous', planHash, baseHead: state.head, branch: state.branch, worktree: state.worktree };
      } else if (input.confirmed !== true) throw new Error('Explicit plan approval required');
      tracking.requireStage(dir, 'intake');
      return progress({ phase: 'approved', next: 'dispatch', approvedPlanHash: planHash, approval, reconciliationRequired: false });
    }
    case 'scope': {
      approved(); reconciled();
      if (input.files) return autonomy.recordScope(dir, input.files, cwd);
      const approval = session.readCheckpoint(repo, task, cwd)?.approval;
      return { approval, changes: autonomy.changes(approval?.baseHead, cwd) };
    }
    case 'progress': {
      const saved = session.readCheckpoint(repo, task, cwd);
      if (input.entry?.mergedPr !== undefined) throw new Error('Merge evidence must come from close');
      if (input.entry?.approval !== undefined || input.entry?.approvedPlanHash !== undefined) throw new Error('Plan authorization must come from approve');
    ...
    case 'status': {
      ...
      const saved = session.readCheckpoint(repo, task, cwd);
      const classification = saved ? session.classifyChange(saved, current, cwd) : 'unchanged';
      const preview = classification === 'divergent' ? { ...saved, verification: 'stale', reconciliationRequired: true, next: 'reconcile' }
        : classification === 'branch_work' ? { ...saved, verification: 'stale', branchWork: true, next: 'result' }
        : saved;
      return { checkpoint: preview, current, tracking: tracking.read(dir), progress: session.readProgress(repo, task, cwd) };
    }
    ...
    case 'route': {
      ...
      const waves = buildExecutionWaves(value.tasks);
      return { waves, auditor: { tier: reviewerTier, model: modelForTier(reviewerTier), risk: scoreRisk(combined) },
    ...
    case 'dispatch': {
      approved(); reconciled();
      autonomy.requireScope(dir, cwd);
      tracking.requireStage(dir, 'start');
      const state = snapshot(cwd);
      if (state.dirtyFiles.length) throw new Error('Commit or reconcile existing changes before dispatch');
      const saved = session.readCheckpoint(repo, task, cwd);
      ...
      // Engineers commit directly on the integration branch, so waves are always one task;
      // this is a defensive rail against a future scheduler regression.
      if (wave.length > 1) throw new Error('Dispatch runs one task at a time');
      const counters = { implementationFailures: saved?.implementationFailures || 0, verificationFailures: saved?.verificationFailures || 0 };
      const routing = wave.map((id) => {
        const item = value.tasks.find((entry) => entry.id === id);
        const risk = { ...item, ...counters };
        const tier = tierForTask('engineer', risk);
    ...
    case 'result': {
      ...
      const entry = { phase: 'engineer_result', consumedResults: [...(saved.consumedResults || []), record.attemptId],
      ...
      // A non-done result may leave un-journaled commits (or a dirty tree) sitting directly
      // on the integration branch; force reconciliation before recover/integrate touch it.
      if (record.status !== 'done') {
        const current = snapshot(cwd);
        const descendsFromBase = current.head === assignment.baseHead
          || (() => { try { git(cwd, ['merge-base', '--is-ancestor', assignment.baseHead, current.head]); return true; } catch (_) { return false; } })();
        if (!descendsFromBase) {
          entry.reconciliationRequired = true;
          entry.reconciliationReason = `Integration branch no longer descends from the dispatched base ${assignment.baseHead}`;
          entry.next = 'reconcile';
        } else {
          const taskDef = plan().tasks.find((item) => item.id === record.taskId);
          const { commits, outside } = ownershipReport(taskDef, assignment.baseHead, current.head, cwd);
          // Each clause only speaks about what it actually checked: a commit clause never
          // appears without commits, a dirty-file clause never appears without dirty files.
          const clauses = [];
          if (commits.length) clauses.push(`left commit(s) ${commits.join(', ')}; committed files ${outside.length ? `outside declared ownership: ${outside.join(', ')}` : 'all within declared ownership'}`);
          if (current.dirtyFiles.length) {
            const dirtyOutside = dirtyOutsideOwnership(taskDef, cwd);
            clauses.push(`left dirty file(s) ${current.dirtyFiles.join(', ')}; dirty files ${dirtyOutside.length ? `outside declared ownership: ${dirtyOutside.join(', ')}` : 'all within declared ownership'}`);
          }
          if (clauses.length) {
            entry.reconciliationRequired = true;
            entry.reconciliationReason = `Attempt ${record.attemptId} ${clauses.join(' ')}`;
            entry.next = 'reconcile';
          }
        }
      }
    ...
    case 'visual-status': return verification.humanConfirmationStatus(dir, cwd);
    case 'human-confirmation': {
      if (input.confirmed !== true) throw new Error('Explicit user confirmation required');
      plan();
      return verification.recordHumanConfirmation(dir, cwd);
    }
    case 'ship': {
      approved(); reconciled();
      tracking.requireStage(dir, 'start');
      ...
      const defaultBranch = originDefaultBranch(cwd);
