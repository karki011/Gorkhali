...
function defaultBranch(cwd) {
  try { return git(cwd, ['symbolic-ref', '-q', '--short', 'refs/remotes/origin/HEAD']).trim().replace(/^origin\//, ''); } catch (_) { return null; }
}
...
function releaseWorktrees(dir, cwd, apply = true) {
  ...
  const protectedBranches = new Set(['main', 'master', branchOf(cwd), defaultBranch(cwd)].filter(Boolean));
  const journal = readJournal(dir);
  const latest = new Map();
  for (const operation of journal) if (operation.sourceWorktree) latest.set(operation.sourceWorktree, operation);
  const released = [];
  const kept = [];
  const unintegrated = [];
  for (const [worktree, operation] of latest) {
    ...
    try {
      ...
      if (branch && (protectedBranches.has(branch) || git(cwd, ['rev-parse', '-q', '--verify', `refs/heads/${branch}`]).trim() !== operation.sourceHead)) branch = null;
      if (apply) {
