...
function integrate(plan, records, dir, cwd) {
  ...
  for (const record of [...records].sort((a, b) => plan.tasks.findIndex((t) => t.id === a.taskId) - plan.tasks.findIndex((t) => t.id === b.taskId))) {
    ...
    if (!operation) {
      ...
      operation = { taskId: task.id, revision, taskHash: record.taskHash, sourceHead: record.head, sourceWorktree: fs.realpathSync(record.worktree), beforeHead: current.head, commits: git(cwd, ['rev-list', '--reverse', `${record.baseHead}..${record.head}`]).trim().split('\n').filter(Boolean), applied: [] };
...
// released: integrated, clean, and exactly at the integrated completion; removed with its branch.
...
function dirtyFiles(worktree) {
  return git(worktree, ['status', '--porcelain', '-z', '--no-renames', '--untracked-files=all']).split('\0').filter(Boolean).map((line) => line.slice(3)).sort();
}
function attemptRecords(dir) {
  ...
  return names.map((name) => ({ file: path.join(folder, name), at: fs.statSync(path.join(folder, name)).mtimeMs })).sort((a, b) => a.at - b.at)
    .map((item) => { try { return JSON.parse(fs.readFileSync(item.file, 'utf8')); } catch (_) { return null; } })
    .filter((record) => record && typeof record.worktree === 'string');
}
function releaseWorktrees(dir, cwd, apply = true) {
  const head = git(cwd, ['rev-parse', 'HEAD']).trim();
  const root = fs.realpathSync(git(cwd, ['rev-parse', '--show-toplevel']).trim());
  const latest = new Map();
  for (const operation of readJournal(dir)) if (operation.sourceWorktree) latest.set(operation.sourceWorktree, operation);
  const released = [];
  const kept = [];
  const unintegrated = [];
  for (const [worktree, operation] of latest) {
    ...
    try {
      ...
      const ref = git(real, ['rev-parse', '--abbrev-ref', 'HEAD']).trim();
      const branch = ref === 'HEAD' ? null : ref;
      if (apply) {
        git(cwd, ['worktree', 'remove', real]);
        if (branch && git(cwd, ['rev-parse', `refs/heads/${branch}`]).trim() === operation.sourceHead) git(cwd, ['branch', '-D', branch]);
      }
      released.push({ worktree: real, branch, taskId: operation.taskId, sourceHead: operation.sourceHead });
    } catch (err) { keep(String(err.stderr || err.message).trim()); }
  }
  // Attempts whose commits were integrated are covered above, or predate the journal's
  // worktree field; either way they are not unintegrated work.
  const integratedHeads = new Set(readJournal(dir).filter((operation) => operation.integratedHead).map((operation) => operation.sourceHead));
  const seen = new Map();
  for (const record of attemptRecords(dir)) seen.set(record.worktree, record);
  for (const [worktree, record] of seen) {
    try {
      if (integratedHeads.has(record.head) || !fs.existsSync(worktree)) continue;
      const real = fs.realpathSync(worktree);
      if (latest.has(real) || latest.has(worktree)) continue;
...
module.exports = { prepareWorktree, completion, integrate, releaseWorktrees, readJournal, taskHash, taskRevisions, completedRecords, ownershipSpec };
