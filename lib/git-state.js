...
function originDefaultBranch(cwd) {
  let remoteHead;
  try { remoteHead = git(cwd, ['ls-remote', '--symref', 'origin', 'HEAD']); }
  catch (_) { throw new Error('Cannot establish origin default branch'); }
  const branch = remoteHead.match(/^ref: refs\/heads\/(.+)\tHEAD$/m)?.[1];
  if (!branch) throw new Error('Cannot establish origin default branch');
  return branch;
}
...
module.exports = { git, digest, snapshot, originDefaultBranch };
