...
function validatePlan(plan) {
  ...
  if (plan.visualReview !== undefined) {
    const value = plan.visualReview;
    if (!isObject(value) || !isNonEmptyString(value.scope) || !isNonEmptyArray(value.checklist) ||
        value.checklist.some((item) => !isNonEmptyString(item))) errors.push('visualReview: a scope and non-empty checklist of expected user-visible behavior are required');
  }
