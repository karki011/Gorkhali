...
const ISOLATION_VALUES = ['auto', 'worktree', 'branch'];

function validateIsolation(value, path, errors) {
  if (value !== undefined && !(typeof value === 'string' && ISOLATION_VALUES.includes(value))) {
    errors.push(`${path}: must be one of ${ISOLATION_VALUES.join(', ')}`);
  }
}
...
function validateTask(task, i, errors) {
  ...
  if (task.parallelSafe !== undefined && typeof task.parallelSafe !== 'boolean') {
    errors.push(`${path}.parallelSafe: must be boolean if present`);
  }
  if (task.coordinationKeys !== undefined) {
    if (!Array.isArray(task.coordinationKeys)) {
      errors.push(`${path}.coordinationKeys: must be array if present`);
    } else if (task.coordinationKeys.some((value) => !isNonEmptyString(value))) {
      errors.push(`${path}.coordinationKeys: values must be non-empty strings`);
    }
  }
  ...
  validateIsolation(task.isolation, `${path}.isolation`, errors);
}
...
function validatePlan(plan) {
  ...
  validateIsolation(plan.isolation, 'isolation', errors);
