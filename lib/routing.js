...
function hasActiveEngineers(state) {
  return Array.isArray(state?.activeEngineers) && state.activeEngineers.length > 0;
}
...
function isolationForTask(task, wave, plan, state = {}) {
  ...
  // Any other pending task, including one that depends on this task or shares a
  // coordination key with it, keeps this task in worktree isolation.
  if (otherPendingIds(task, state).length > 0) return 'worktree';
