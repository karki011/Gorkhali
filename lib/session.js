...
function checkpoint(repo, task, entry = {}, cwd = process.cwd()) {
  ...
  if (entry.approval !== undefined) value.approval = entry.approval;
 ...
 * branch and worktree and every active engineer's assignment has a base that
 * is HEAD or an ancestor of it, meaning the change is that Engineer's own
 * in-progress commits on the integration branch rather than external drift;
 * `divergent` otherwise, the general "something else touched Git" case.
 */
