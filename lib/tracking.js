...
function nextAction(state, observation) {
  ...
  if (stage === 'start') {
    ...
    if ((!observation.assignees.length || state.reassign) && !text(desired)) throw new Error('Resolve the authenticated tracker user before assigning');
    if ((!observation.assignees.length || state.reassign) && !observation.assignees.includes(desired)) return { kind: 'assign', assignee: desired, replace: state.reassign };
  }
  if (stage === 'review') {
  ...
  const names = state.statusNames[stage] ? [state.statusNames[stage]] : STATUS_NAMES[stage];
  const match = (name) => names.some((value) => value.toLowerCase() === name?.toLowerCase());
  if (match(observation.status.name)) {
    if (state.ticket.provider === 'github' && stage === 'done' && !observation.closed) return { kind: 'close' };
    return null;
  }
  const candidates = (observation.transitions || []).filter((entry) => text(entry.id) && match(entry.name));
