...
const { snapshot, digest, git } = require('./git-state');
...
function visualScopeHash(dir) {
  const plan = read(path.join(dir, 'plan.json'));
  if (!plan) throw new Error('Visual review requires the session plan');
  return digest(JSON.stringify(plan.visualReview || plan));
}
function recordHumanConfirmation(dir, cwd) {
  const state = snapshot(cwd);
  if (state.dirtyFiles.length) throw new Error('Integrate and commit work before recording visual confirmation');
  const record = { confirmed: true, fingerprint: state.fingerprint, head: state.head, branch: state.branch,
    worktree: state.worktree, visualScopeHash: visualScopeHash(dir), planHash: planHash(dir) };
  writeJsonAtomic(path.join(dir, 'human-confirmation.json'), record);
  return record;
}
function humanConfirmationStatus(dir, cwd) {
  const human = read(path.join(dir, 'human-confirmation.json'));
  if (human?.confirmed !== true) return { reusable: false, reason: 'No human visual confirmation has been recorded' };
  const state = snapshot(cwd);
  if (human.visualScopeHash && human.visualScopeHash !== visualScopeHash(dir)) return { reusable: false, reason: 'The visual acceptance scope changed' };
  if (human.fingerprint === state.fingerprint) return { reusable: true, basis: 'same-state' };
  if (!human.visualScopeHash) return { reusable: false, reason: 'Legacy confirmation has no recorded visual scope for reuse' };
  if (state.dirtyFiles.length || human.branch !== state.branch || human.worktree !== state.worktree) return { reusable: false, reason: 'Reconcile the checkout before reusing visual confirmation' };
  if (!/^[a-f0-9]{40,64}$/.test(human.head || '')) return { reusable: false, reason: 'Visual confirmation has no valid base commit' };
  try { git(cwd, ['merge-base', '--is-ancestor', human.head, state.head]); }
  catch (_) { return { reusable: false, reason: 'Current work does not descend from the visually confirmed commit' }; }
  return { reusable: true, basis: 'same-visual-scope', confirmedHead: human.head };
}
...
function recordAuditor(dir, record, cwd) {
  ...
  const value = { ...record, role: 'auditor', id: randomUUID(), planHash: inspector.planHash };
...
function requireVerified(dir, cwd) {
  ...
  const requireHuman = (record) => {
    ...
    const status = humanConfirmationStatus(dir, cwd);
    if (!status.reusable) throw new Error(`Explicit human visual confirmation required: ${status.reason}`);
  };
  if (current) {
  ...
  if (auditor.planHash !== inspector.planHash) throw new Error('Changed or unknown plan scope requires a current Auditor review');
  requireHuman(auditor);
  return { inspector, auditor: null, auditorWaived: reason };
}
module.exports = { recordInspector, requireInspector, recordAuditor, requireVerified, auditorWaiver, recordHumanConfirmation, humanConfirmationStatus };
