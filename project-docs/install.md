...
| Host | Project path |
|---|---|
| Claude Code | `.claude/skills/gorkhali/` |
| Codex | `.agents/skills/gorkhali/` |
| Kimi Code | `.agents/skills/gorkhali/` |
| Gemini CLI | `.agents/skills/gorkhali/` |

The copied `gorkhali` directory and every file inside it, including the canonical
...
Use each host's user-level skills directory instead when you want Gorkhali in
every project.
...
```bash
git clone git@github.com:karki011/Gorkhali.git
cd Gorkhali
codex
# Then, inside Codex:
/plugins
...
Select the repository marketplace, open `gorkhali`, and choose **Install**. Codex
...
exposes `./skills/` (every `gorkhali:*` workflow), ships the 11-role roster under
...
CLI has no per-spawn model selector, so Gorkhali's profile resolution is
...
plugin manager, copy `skills/gorkhali` into `.agents/skills/gorkhali/` as shown
...
agents, and hooks directly. Gorkhali creates mutable state and per-repository
...
checkout, open `/plugins`, uninstall and reinstall `gorkhali`, then start a new
...
Claude Code users can run `/plugin update gorkhali`.
...
`~/.claude/commands/gorkhali` and `~/.claude/agents/gorkhali` entries so Claude
Code cannot discover a stale copy alongside the plugin. The old flow may also
have registered these four Gorkhali hooks in `~/.claude/settings.json`; remove
...
Back up `settings.json` before editing and preserve every non-Gorkhali hook. If
you need data from an old `~/.claude/team`, `~/.claude/gorkhali`, or
`~/.claude/gorkhali-data` directory, the optional `scripts/migrate-data.js`
utility copies its data whitelist into `GORKHALI_DATA` (or `~/.gorkhali` when
unset) without modifying the source. Pre-existing destination entries always
win; otherwise legacy collisions use `gorkhali-data`, then `gorkhali`, then
