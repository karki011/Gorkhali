# Optional Surveyor authentication protocol

Load this reference only when an explicitly requested Surveyor route reaches an
authentication wall.

1. Prefer the existing authenticated `surveyor-qa` browser session.
...
an advisory Surveyor gap, not a failed user-verification decision.
