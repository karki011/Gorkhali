...
- **`plan.json`** — the machine source of truth, and the only gate surface. `gorkhali:execute`, `gorkhali:wire`, and `gorkhali:resume` all read this file. Every new plan sets `_meta.version: 3` and `depth` to `quick`, `standard`, or `deep`.
...
If `plan.json` changes after the initial review — during deliberation, a fix-loop revision, or a resumed session — present the updated chat brief before the next requested human review.

### Gate-loop revisions

During plan-gate chat feedback, apply material feedback to `plan.json`; presentation-only feedback leaves the JSON unchanged and only the presented brief regenerates. Re-run Opposition for a material change before a requested re-review. Record each material revision in `{SESSION_DIR}/decisions.json`, including the feedback, plan changes, and recheck result. Chat approval remains the only gate exit.
