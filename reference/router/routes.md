...
- Inspector-only verification: one Inspector spawn writes verification.json.
  Does NOT chain into `gorkhali:verify --chained` (Steward/Justice/Auditor skipped)
- Records the portable lifecycle transitions it performed (authorize implementation,
  execute, record --type verification) via gorkhali-state.mjs CLI writes — status,
  resume, and wrap must see the LITE pass; see commands/start.md's LITE section
- If Inspector FAILS -> chain to `gorkhali:fix` (fix-loop ceiling unchanged)
- On PASS -> report and stop; no auto-wrap. `/gorkhali:wrap` still requires the
  full `/gorkhali:verify` review pass first (its ship gate needs an Auditor review)
...
- Artifacts: context.json, route-decision.json (route: "LITE"), verification.json,
  plus the portable lifecycle record (`runs/<run>/verification.json` via gorkhali-state.mjs)
...
## BRAINSTORM
```
Context -> Router(BRAINSTORM) -> Skill(skill="gorkhali:brainstorm")
  -> Diverge (explore + questions + 2-3 approaches)
  -> Converge (human picks direction, decision locked)
-> [net_new_ui: Skill(skill="gorkhali:visualflow")] -> Standard PLAN flow (decompose -> deliberate -> approve -> execute -> verify)
```
- 2 human gates: approve direction (in brainstorm) + approve plan
- Brainstorm phase invoked via `Skill(skill="gorkhali:brainstorm")` -- see `commands/brainstorm.md`
- **Visual flow**: when `net_new_ui` fires, before contracts/plan -> `Skill(skill="gorkhali:visualflow")` (visual flow, user-gated) -- Chief recommends, user approves; no new hard gate
- Artifacts: + decisions.json (from brainstorm), intent.json (updated with chosen approach), visualflow.json (when net_new_ui)

## FULL
```
Context -> Router(FULL) -> Skill(skill="gorkhali:brainstorm") (diverge/converge)
-> Direction locked -> [net_new_ui: Skill(skill="gorkhali:visualflow")] -> PLAN (decompose/deliberate) -> Plan approved
-> Skill(skill="gorkhali:wire") (dependency topology, wave assignments, risk points) -> Human approves wiring
-> EXECUTE (wave-based dispatch) -> VERIFY -> Done
```
- 3 human gates: direction + plan + wiring
- Brainstorm invoked via `Skill(skill="gorkhali:brainstorm")` -- see `commands/brainstorm.md`
- **Visual flow**: when `net_new_ui` fires, before contracts/plan -> `Skill(skill="gorkhali:visualflow")` (visual flow, user-gated) -- Chief recommends, user approves; no new hard gate
- Wiring invoked via `Skill(skill="gorkhali:wire")` -- see `commands/wire.md` and `reference/wiring.md`
