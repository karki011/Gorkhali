...
Flags net-new UI work with no design to follow, so Chief RECOMMENDS a visual flow pass before
contracts/plan. Chief cannot self-launch — it recommends; the user gates the flow.
...
shadows. Chief cannot self-launch — it recommends; the user triggers. Full test + recommend pattern:
`reference/workflow-delegation.md`.
- **Primary lever: SCALE** — >= ~20 files blast radius, OR >= ~5 sources/angles, OR deep
  git-history steward, OR "codebase-wide" / "every X" phrasing.
