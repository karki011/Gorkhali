...
Gorkhali's routing system has two layers: an advisory nudge that fires at prompt time, and an opt-in enforcement gate that fires at edit time. Neither layer is required for Gorkhali to function — the nudge alone is the default, and it cannot block anything.
...
| 2 — Gate | `routing-gate.js` | `PreToolUse` | Denies Edit/Write/MultiEdit/NotebookEdit in Gorkhali-known repositories when no matching repository-scoped portable session is active. Fires only when `GORKHALI_ROUTING_ENFORCE=1`; `GORKHALI_ROUTING_SCOPE=all-git` explicitly expands its scope. |

Layer 1 is always on (unless `GORKHALI_ROUTING_NUDGE=0`). Layer 2 is off by default; it requires an explicit opt-in.
...
One-shot semantics: a marker is written to `<GORKHALI_DATA>/state/routing-nudge/<session_id>`. Subsequent matching prompts in the same session are silent. If the marker write fails, the hook fails toward one more emit — never toward silence.
...
1. Is `GORKHALI_ROUTING_ENFORCE=1` set in the environment? → if not, allow.
2. Is the target outside a Git repository or inside Gorkhali's own data tree? → allow.
3. Is the repository unknown to Gorkhali and `GORKHALI_ROUTING_SCOPE` is not `all-git`? → allow.
...
|------|------|-------|-------------------------------|
| `routing-gate.js` | Discipline (opt-in) | **Open** — crash or any unset/unrecognized value of `GORKHALI_ROUTING_ENFORCE` all allow the edit | Blocking legitimate work because the gate misbehaved would be worse than a missed routing event. The cost of a false-positive deny is high; the cost of a false-negative allow is a process note. An unset env var can **never** enable the gate — only `GORKHALI_ROUTING_ENFORCE=1` arms it. |
...
Enforcement defaults to off. If `GORKHALI_ROUTING_ENFORCE` is unset or anything other than `1`, the gate exits immediately. No path other than an explicit `GORKHALI_ROUTING_ENFORCE=1` can arm the gate.

---

## Honest Efficacy

By default (`GORKHALI_ROUTING_ENFORCE` unset), the gate never fires. The missed-routing incident class — implementation work that bypasses gorkhali and runs directly — is mitigated only by the advisory nudge in Layer 1. The deterministic enforcement layer exists for operators who explicitly set `GORKHALI_ROUTING_ENFORCE=1` after reviewing the bypass log. If you haven't done that, the gate is documentation, not enforcement.

---

## Scoping — Repository and Worktree Bound

When enforcement is enabled, the gate covers project-file edits in Gorkhali-known Git repositories. The repository is resolved by walking up from the edit target until a `.git` entry is found; linked-worktree `.git` files count. Set `GORKHALI_ROUTING_SCOPE=all-git` to explicitly include repositories Gorkhali has never managed.

Non-repository files and Gorkhali's mutable data tree remain outside the gate. `GORKHALI_ADHOC=1` remains the explicit logged bypass for deliberate ad-hoc repository work.

### Active Session Detection

A Gorkhali session satisfies routing only when the canonical repository identity resolves to `<GORKHALI_DATA>/state/current-session/<repo-id>.json`, that pointer references a session under the same repository's `sessions/` directory, and `session.json` is schema 1, active, task-matched, repository-matched, and identity-root-matched. The gate mirrors the portable lifecycle engine's identity root instead of inventing a second worktree rule: no-origin linked worktrees resolve through their shared Git common root, while remote-backed worktrees retain distinct checkout roots and cannot unlock sibling worktrees.
...
## Bypass — GORKHALI_ADHOC=1

Set `GORKHALI_ADHOC=1` in the environment before running Claude to allow edits in enforce mode without a gorkhali session. The gate allows the edit and appends a record to `<GORKHALI_DATA>/state/routing-bypass.jsonl`:
...
**Rollout**: leave `GORKHALI_ROUTING_ENFORCE` unset initially, review the bypass log to understand ad-hoc edit patterns, then set it to `1`. Add `GORKHALI_ROUTING_SCOPE=all-git` only when every Git repository should be covered.
...
`routing-nudge/<session_id>` markers accumulate in `<GORKHALI_DATA>/state/routing-nudge/` over time — one file per host session that received a nudge. There is no current cleanup mechanism. This directory is a future candidate for `session-cleanup.js`.
