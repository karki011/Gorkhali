...
Written by `gorkhali:verify`. Read by `gorkhali:wrap` to decide PR strategy.
...
`{LINT_CMD}` / `{BUILD_CMD}` / `{TEST_CMD}` are resolved via the discovery protocol in `skills/gorkhali/references/verification.md`. The real file records the concrete commands actually run.
