// Author: Subash Karki
// axi-error.js - typed error + deterministic exit-code mapping for Gorkhali CLIs.
// A GorkhaliError carries a machine code and optional remediation suggestions;
// exitCodeForError maps it to a process exit status. Callers set
// `process.exitCode` (or throw a GorkhaliError caught by reportError) and RETURN -
...
// error and a single exit-code decision; this port renames it GorkhaliError, pins
// the taxonomy at VALIDATION_ERROR -> 2 / everything else -> 1, and adds a shared
// reportError sink so the Gorkhali CLIs share one non-absorbing backstop.
'use strict';

const VALIDATION_ERROR = 'VALIDATION_ERROR';

class GorkhaliError extends Error {
  constructor(message, code, suggestions = []) {
    super(message);
    this.name = 'GorkhaliError';
    this.code = code;
...
function isGorkhaliError(err) {
  return err instanceof GorkhaliError || (err != null && err.name === 'GorkhaliError');
}
...
// GorkhaliError prints just its message (already user-facing); anything else
// prints its stack so an unexpected failure is loud, not swallowed.
function reportError(err, stream = process.stderr) {
  const gorkhali = isGorkhaliError(err);
  const body = gorkhali ? err.message : (err && err.stack) || String(err);
...
module.exports = { GorkhaliError, exitCodeForError, reportError, isGorkhaliError, VALIDATION_ERROR };

const HELP =
  'axi-error - typed error + exit-code taxonomy for Gorkhali CLIs\n\n' +
...
if (require.main === module) {
  ...
  if (arg === undefined || arg === '--help') {
    ...
    const code = exitCodeForError(new GorkhaliError('demo', arg));
