...
if [ -L "$COMMANDS_LINK" ] || [ -d "$COMMANDS_LINK" ]; then
  ...
  echo "  ✓ Created symlink: $COMMANDS_LINK → $PHANTOM_DIR/commands"
fi

# 2. Initialize directories
for dir in sessions events state state/completed learnings global/patterns audit; do
  mkdir -p "$PHANTOM_DIR/$dir"
  if [ ! -f "$PHANTOM_DIR/$dir/.gitkeep" ]; then
    touch "$PHANTOM_DIR/$dir/.gitkeep"
  fi
done
echo "  ✓ Directories initialized (sessions, events, state, learnings, global/patterns)"

# 3. Initialize learnings INDEX.md if empty
if [ ! -f "$PHANTOM_DIR/learnings/INDEX.md" ]; then
  cat > "$PHANTOM_DIR/learnings/INDEX.md" << 'INDEXEOF'
# Learnings Index
...
# 6. Write config
cat > "$PHANTOM_DIR/config.yaml" << CONFIGEOF
...
CONFIGEOF
echo ""
echo "  ✓ Config written to $PHANTOM_DIR/config.yaml"
...
for hook in "$PHANTOM_DIR"/hooks/*.sh "$PHANTOM_DIR"/hooks/*.js; do
  [ -f "$hook" ] && chmod +x "$hook"
done
echo "  ✓ Hook scripts ready"

for script in "$PHANTOM_DIR"/scripts/*.sh "$PHANTOM_DIR"/scripts/*.js; do
